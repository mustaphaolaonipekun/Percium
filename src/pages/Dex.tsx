...
import {
  ...
  fetchAllPositions,
  fetchUserAccount,
  openPosition as openPositionOnChain,
  closePosition as closePositionOnChain,
  ensureUserAccount,
  readProtocolBalance,
  LAMPORTS_PER_USDC,
  toBN,
...
export default function Dex() {
  ...
  // Fetch positions — no AnchorProvider, uses raw account reads
  const loadPositions = async () => {
    if (!wallet.publicKey) return
    try {
      const userAcc = await fetchUserAccount(connection, wallet.publicKey)
      if (!userAcc) return
      const total = userAcc.totalPositions.toNumber()
      const all = await fetchAllPositions(connection, wallet.publicKey, total)
      // Map to the shape the table expects (with liquidationPrice and pnl as BN)
      const mapped = all.map(pos => {
        const entry  = pos.entryPrice.toNumber() / LAMPORTS_PER_USDC
        const col    = pos.collateral.toNumber() / LAMPORTS_PER_USDC
        const liqRaw = pos.side === 0
          ? entry * (1 - 1 / pos.leverage * 0.9)
          : entry * (1 + 1 / pos.leverage * 0.9)
        const pnlRaw = pos.side === 0
          ? (displayPrice - entry) * col * pos.leverage / entry
          : (entry - displayPrice) * col * pos.leverage / entry
        return {
          ...pos,
          pda: pos.publicKey,
          status: pos.isOpen ? 0 : 1,
          liquidationPrice: toBN(liqRaw * LAMPORTS_PER_USDC),
          pnl: toBN(pnlRaw * LAMPORTS_PER_USDC),
        }
      })
      setPositions(mapped)
    } catch (e) { console.warn('loadPositions:', e) }
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      // Ensure user account exists — no AnchorProvider ✓
      await ensureUserAccount(connection, wallet as any)

      // Get current position index from on-chain account
      const userAcc = await fetchUserAccount(connection, pk)
      const posIndex = userAcc ? userAcc.totalPositions : new BN(0)

      const mktSide: 0|1 = side === 'buy' ? 0 : 1
      const mktKey = MARKETS.find(m => m.label === market)?.key ?? 'SOL'

      // ✓ No AnchorProvider — wallet adapter passed directly
      const { txSig } = await openPositionOnChain(
        connection, wallet as any,
        mktKey, mktSide, leverage, col, displayPrice, posIndex
      )
  ...
  const handleClose = async (pos: any) => {
    ...
    try {
      // ✓ No AnchorProvider — wallet adapter passed directly
      const txSig = await closePositionOnChain(connection, wallet as any, pos.pda, displayPrice)
      setTxLink(`https://solscan.io/tx/${txSig}?cluster=devnet`)
      showToast('✓ Position closed. PnL settled on-chain.', 'success')
      await loadPositions()
      await refreshAfterTx(2500)
        ...
        <div style={{ flex:1, display:'flex', minHeight:0 }}>
          ...
          <div style={{ width:288, flexShrink:0, display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ flex:1, padding:15, overflowY:'auto' }}>
              ...
              <input type="range" min={1} max={20} value={leverage} onChange={e => setLeverage(+e.target.value)} style={{ width:'100%', marginBottom:4 }} />
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'#4a6e5a', marginBottom:14 }}>
                {['1×','5×','10×','15×','20×'].map(l => <span key={l}>{l}</span>)}
              </div>
              ...
              {/* Arcium privacy badge */}
              <div style={{ background:'rgba(124,77,255,0.08)', border:'1px solid rgba(124,77,255,0.2)', borderRadius:8, padding:'9px 12px', fontSize:11, color:'#b39ddb', lineHeight:1.5 }}>
                🔒 <strong>Encrypted via Arcium MXE</strong>. Position, collateral &amp; liquidation price are private on-chain.
              </div>
            </div>
          </div>
        </div>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              ...
              <tbody>
                ...
                {shownPositions.map((pos,i) => {
                  ...
                  const col = pos.collateral.toNumber() / LAMPORTS_PER_USDC
                  const pnl = pos.pnl ? pos.pnl.toNumber() / LAMPORTS_PER_USDC : 0
                  return (
                    <tr key={i} style={{ borderBottom:'1px solid rgba(255,255,255,0.03)' }}>
                      <td style={{ padding:'6px 14px', fontSize:12, color:'#e8f5e9', fontWeight:600 }}>{pos.market}</td>
                      <td style={{ padding:'6px 14px', fontSize:12, color:pos.side===0?'#00e676':'#ef5350', fontWeight:700 }}>{pos.side===0?'LONG':'SHORT'}</td>
